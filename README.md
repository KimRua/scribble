...
- `ethers` 기반 opBNB onchain proof 기록 경로
...
- 주문 실행은 여전히 mock 체결이지만, 설정이 있으면 opBNB `ExecutionRegistry`에 proof 트랜잭션을 기록합니다.
- 온체인 기록은 컨트랙트 샘플과 ABI 검증까지 구현되어 있고, 실제 체인 송신은 다음 단계입니다.

## 다음 완성 단계

1. Binance WebSocket 직접 연동으로 초저지연 스트림 전환
2. 실제 LLM 키 주입
