...
class CanvasTxt {
  ...
  resize() {
    ...
    const textWidth =
      Math.ceil((metrics.actualBoundingBoxLeft || 0) + (metrics.actualBoundingBoxRight || metrics.width)) + 24;
  ...
  render() {
    ...
    for (let y = 0; y < this.canvas.height; y += 1) {
      for (let x = 0; x < this.canvas.width; x += 1) {
        const index = (y * this.canvas.width + x) * 4 + 3;
    ...
    if (right >= left) {
      ...
      this.visualCenterOffsetX = this.canvas.width / 2 - visualCenter;
    } else {
      this.visualCenterOffsetX = 0;
    }
  }
...
class CanvAscii {
  ...
  fitMeshToViewport() {
    ...
    const safeWidth = visibleWidth * 0.78;
    const safeHeight = visibleHeight * 0.4;

    const widthScale = safeWidth / this.basePlaneWidth;
    const heightScale = safeHeight / this.basePlaneHeight;
    const fitScale = Math.min(widthScale, heightScale, 1);
    const offsetRatio = this.textCanvas.width > 0 ? this.textCanvas.visualCenterOffsetX / this.textCanvas.width : 0;

    this.mesh.scale.setScalar(fitScale);
    this.centerOffsetX = this.basePlaneWidth * fitScale * offsetRatio;
    this.mesh.position.set(this.centerOffsetX, 0, 0);
  }
  ...
  onMouseMove(evt: MouseEvent | TouchEvent) {
    ...
    const e = touchEvent.touches?.[0] ?? (evt as MouseEvent);
  ...
  updateRotation() {
    ...
    this.mesh.position.x += (this.centerOffsetX - this.mesh.position.x) * 0.08;
    this.mesh.position.y += (0 - this.mesh.position.y) * 0.08;
  }
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const setup = async () => {
        ...
        observer = new IntersectionObserver(
          async ([entry]) => {
            ...
            if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
              ...
              if (!cancelled && currentContainer) {
                asciiRef.current = await createAndInit(currentContainer, w, h);
      ...
      asciiRef.current = await createAndInit(container, rect.width, rect.height);
      if (!cancelled && asciiRef.current) {
        asciiRef.current.load();
